...
if __name__ == '__main__':
    ...
    param_min = float(np.min(param_range))
    param_max = float(np.max(param_range))
                ...
                for sent_id in ds[lang]:
                    ...
                    for w_i, wps in enumerate(tokenized['input_ids']):
                        ...
                        for wp in wps:
                            ...
                            normalized_label = (orig_labels[w_i] - param_min) / (param_max - param_min)
                            new_labels.append(normalized_label) # TODO labels for subsequent labels use -100, check loss default
    ...
    lang_adapter_hub_name = single_lang + "/wiki@ukp"
    model.load_adapter(lang_adapter_hub_name, config=lang_adapter_config)
    ...
    model.active_adapters = Stack(single_lang, adapter_name_lang)

    # only train the task adapter
    model.train_adapter([adapter_name_lang])
    ...
    training_args = TrainingArguments(
        ...
        logging_steps=200,
        output_dir="./training_output_roberta_xlm_stack_" + parameter + "_" + single_lang + '_' + str(seed) + '_' + str(learning_rate) + '_'
    ...
    def compute_accuracy(p: EvalPrediction):
      preds = p.predictions
      return {"acc": (((np.abs(preds.squeeze() - p.label_ids)) * (param_max - param_min) + param_min)).mean()}


    subset_size = 4 # -1 # TODO
    ...
    model.save_adapter("./final_adapter_roberta_xlm_stack_" + parameter + "_" + single_lang + '_' + str(seed) + '_' + str(learning_rate) + '_'
    ...
    for split in datapaths_tests:
        ...
        if split == 'test1':
            test_text = parameter + '_test_1_xlm_' + single_lang + '_' + str(seed) + '_' + str(learning_rate) + '_' \
                        + str(batch_size) + '_' + str(epochs) + '_stack.txt'
        elif split == 'test2':
            test_text = parameter + '_test_2_xlm_' + single_lang + '_' + str(seed) + '_' + str(learning_rate) + '_' \
                        + str(batch_size) + '_' + str(epochs) + '_stack.txt'
                ...
                for sent_id in dataset_param[lang]:
                    ...
                    for lm in label_map:
                        ...
                        if len(label_map[lm]) > 1:
                            ...
                            # multiple wordpieces
                            label_map[lm] = np.mean(label_map[lm]) * (param_max - param_min) + param_min

                        else:
                            label_map[lm] = label_map[lm][0] * (param_max - param_min) + param_min
