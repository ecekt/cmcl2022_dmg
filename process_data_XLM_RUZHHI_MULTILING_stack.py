...
if __name__ == '__main__':
                    ...
                    if split in ['train', 'val']:
                        ...
                        if language == single_lang:
                            ...
                            if parameter == 'ffd_avg':
                                param = np.log(float(FFDAvg))
                            elif parameter == 'ffd_std':
                                param = np.log(float(FFDStd))
                            elif parameter == 'trt_avg':
                                param = np.log(float(TRTAvg))
                            elif parameter == 'trt_std':
                                param = np.log(float(TRTStd))
    ...
    def compute_accuracy(p: EvalPrediction):
      preds = p.predictions
      return {"acc": (np.abs(np.exp(preds.squeeze()) - np.exp(p.label_ids))).mean()}
